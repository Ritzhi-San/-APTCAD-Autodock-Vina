...
#include <exception>
#include "parse_error.h" // handle PDBQT file parsing errors
#include <boost/program_options.hpp> // Allows use of the program_options module of the Boost C++ library to simplify command line argument parsing and handling
#include "vina.h" // Perform molecular docking calculations and evaluate interactions between molecules
#include "utils.h" // Provides common functional functions, such as file path processing and output information
...
// Define a struct to track the occurrence of command-line options
struct options_occurrence {
	...
	options_occurrence& operator+=(const options_occurrence& x) {
		some = some || x.some;	// At least one condition is met
		all  = all  && x.all; // All conditions are met
		return *this;
	}
};
...
struct Pose {
	...
	Pose(const std::string& info) : info(info) {}
};

// Function to get the number of occurrences of options in command line arguments
...
// Function to check if all required options are present
// Helps catch missing required options and provides error information about missing options, 
...
#################################################################\n";
	...
	try {
		// Standard variables and setting some initial data
		...
		// Parse command-line arguments and store the parsed results in 'vm'
		try {
			//store(parse_command_line(argc, argv, desc, command_line_style::default_style ^ command_line_style::allow_guessing), vm);
			store(command_line_parser(argc, argv)
				.options(desc) // Parse command-line arguments according to the definition in desc
				.style(command_line_style::default_style ^ command_line_style::allow_guessing)
				.positional(positional) // handle positional arguments
				.run(),
				vm);
			notify(vm);
		...
		// If the configuration file exists, save the data to the "vm"
		// This program does not include create file function
		if (vm.count("config")) { // Calculate the number of "config", if higher than 0, then the program will process
			try {
		...
		// Make sure "receptor" and "maps" are not specified at the same time
		if (vm.count("receptor") && vm.count("maps")) { // Calculate the number "receptor" and "maps", if they are both > 0, then the program will process
		...
		if (sf_name.compare("vina") == 0 || sf_name.compare("vinardo") == 0) {
			// Based on the code above to ensure that "receptor" and "maps" are not specified at the same time
			//  Here, make sure that one of the receptor and maps is specified
			//  If neither is specified, ERROR will be displayed
		...
		if (!vm.count("ligand") && !vm.count("batch")) {
			...
			if (!is_directory(out_dir)) { // Check if "out_dir" isn't an existing directory
		...
		if (verbosity > 0) {
			...
			if (!vm.count("maps") && !autobox) { // "autobox" controls whether the grid is defined automatically, rather than relying on the user to manually specify grid parameters
		...
		// Creates an instance of the Vina class named "v" and stores the supplied parameters in the instance
		// This instance is typically used to perform molecular docking calculations
		...
		if (sf_name.compare("vina") == 0) {
			...
			// Without it, the map file will not be generated
			// Map file can be used to describe the spatial properties of molecules 
			//	to aid in computational tasks of molecular simulations
		...
		if (vm.count("ligand")) {
			...
			// Provides different molecular docking calculation methods
			// To enable researchers to select molecular docking computational methods that fit their specific research needs
			if (randomize_only) {
				v.randomize();
			...
			// Create an unordered map that tracks the number of occurrences of file names
			std::unordered_map<std::string, int> instance_counter;

			std::size_t failed_ligand_parsing = 0; // initialization
			VINA_RANGE(i, 0, batch_ligand_names.size()) { // Use "VINA_RANGE" to loop through the names of a batch of ligand files
				name = get_filename(batch_ligand_names[i]);
				// If the file name is repeated, then add a unique number to the output filename to prevent overwriting an existing file
				...
				// Load individual molecular ligand files from the batch into the 'v' object one by one
				// Try to parse the molecule ligand file
				// If parsing fails, logs an error message and continues processing the next file
				try {
				...
				// The first execution is used for "ligand" option
				// The second execution is used for "batch" option
				if (randomize_only) {
					v.randomize();
					v.write_pose(out_name);
				} else if (score_only) {
	...
	// Maybe the conditions are just not achieved
	// These exceptions typically result from logical issues or coding errors in the program, 
	//  and they should not occur under normal circumstances, thus requiring special attention and correction
