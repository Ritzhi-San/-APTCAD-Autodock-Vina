...
void Vina::global_search(const int exhaustiveness, const int n_poses, const double min_rmsd, const int max_evals) {
	...
	if (!poses.empty()) {
		...
		if (m_verbosity > 0) {
			std::cout << '\n';
			outputBuffer << "mode |   affinity | dist from best mode\n";
			outputBuffer << "     | (kcal/mol) | rmsd l.b.| rmsd u.b.\n";
			outputBuffer << "-----+------------+----------+----------\n";
		}
		...
		VINA_FOR_IN(i, poses) {
			...
			if (m_verbosity > 0) {
				outputBuffer << std::setw(4) << i + 1 << "    " << std::setw(9) << std::setprecision(4) << poses[i].e;
				outputBuffer << "  " << std::setw(9) << std::setprecision(4) << poses[i].lb;
				outputBuffer << "  " << std::setw(9) << std::setprecision(4) << poses[i].ub << "\n";
			}
		}

		// Get the captured output as a string
		outputString = outputBuffer.str();
...
// In order to allow main.cpp to obtain the outputString data
// The reason for adding is for the function of writing log.txt
std::string Vina::getOutputString() const {
	return outputString;
}
